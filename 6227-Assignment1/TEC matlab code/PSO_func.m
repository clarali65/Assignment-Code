...
[gbestval,gbestid]=min(pbestval);%initialize gbest index and the gbest's fitness value
gbest=pbest(gbestid,:);%initialize the gbest
...
for i=2:me%me=Max_Gen
    
    for k=1:ps%ps=Particle_Number

    aa(k,:)=cc(1).*rand(1,D).*(pbest(k,:)-pos(k,:))+cc(2).*rand(1,D).*(gbestrep(k,:)-pos(k,:));%learning rate
...
%     if (sum(pos(k,:)>VRmax(k,:))+sum(pos(k,:)<VRmin(k,:)))==0;
    ...
    tmp=(pbestval(k)<e(k));
    temp=repmat(tmp,1,D);
    pbest(k,:)=temp.*pbest(k,:)+(1-temp).*pos(k,:);%update the pbest
    pbestval(k)=tmp.*pbestval(k)+(1-tmp).*e(k);%update the fitness value of the pbest
